            ...
            <div class="col-6 col-md-3">
                <a href="{{ route('reservasi.denah', ['kategori' => $kategori, 'lantai' => $l->id_lantai, 'jenis' => $jenis?->id_jenis_sewa]) }}"
                   class="xcard hover text-center p-4 h-100" style="border-bottom:4px solid {{ $w }}">
                    <div class="mx-auto mb-2" style="width:3.6rem;height:3.6rem;border-radius:1.1rem;display:grid;place-items:center;background:{{ $w }};color:#fff;font-weight:800;font-size:1.3rem;font-family:'Plus Jakarta Sans',sans-serif;box-shadow:0 10px 20px {{ $w }}44">{{ $l->nomor_lantai }}</div>
